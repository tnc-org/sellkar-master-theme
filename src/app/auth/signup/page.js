...
import { firstNamePattern, lastNamePattern, emailPattern, passwordPattern } from '@/lib/utils/core';
...
export default function SignUpPage() {
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      localStorage.setItem('emailOtp',    otpRes?.data?.data ?? '');
    ...
    <AuthLayout title="Sign up">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <button type="submit" disabled={isLoading}
          className="w-full h-10 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer mt-1">
          Sign Up
        </button>
